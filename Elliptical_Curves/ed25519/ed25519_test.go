...
func BenchmarkBlindKeyPublicKey(b *testing.B) {
	var zero zeroReader
	public, _, err := GenerateKey(zero)
	if err != nil {
		b.Fatal(err)
	}

	blind := make([]byte, PrivateKeySize)
	rand.Reader.Read(blind)

	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _ = BlindPublicKey(public, blind)
	}
}
...
func BenchmarkUnblindPublicKey(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _ = UnblindPublicKey(blindedKey, blind)
	}
}
...
func BenchmarkBlindKeySign(b *testing.B) {
	...
	blind := make([]byte, PrivateKeySize/2) // Set the blind length to PrivateKeySize/2 (32 bytes)
	rand.Reader.Read(blind)

	message := []byte("Hello, world!")
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = BlindKeySign(private, message, blind)
	}
}
